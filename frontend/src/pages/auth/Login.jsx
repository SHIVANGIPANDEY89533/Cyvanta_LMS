...
const Login = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (!err.response) {
        setError('Cannot connect to backend. Is the Java server running?');
      } else {
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'grid', placeItems: 'center', padding: '1rem' }}>
      ...
      <article className="panel" style={{ width: '100%', maxWidth: '420px', padding: 'var(--space-6)' }}>
        ...
        {/* Tab Switcher for Student / Admin */}
        <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1.5rem', background: 'var(--color-surface-2)', padding: '0.3rem', borderRadius: '0.5rem' }}>
          <button 
            ...
            style={{
              ...
              border: 'none',
              cursor: 'pointer',
              background: loginType === 'student' ? 'var(--color-primary)' : 'transparent',
              color: loginType === 'student' ? '#fff' : 'var(--color-text)',
          ...
          <button 
            ...
            style={{
              ...
              border: 'none',
              cursor: 'pointer',
              background: loginType === 'admin' ? 'var(--color-primary)' : 'transparent',
              color: loginType === 'admin' ? '#fff' : 'var(--color-text)',
        ...
        {error && (
          <div style={{ color: 'red', marginBottom: '1rem', textAlign: 'center', fontSize: '0.9rem', padding: '0.5rem', background: 'rgba(255,0,0,0.1)', borderRadius: '0.3rem' }}>
            {error}
          </div>
        )}
        ...
        <form onSubmit={handleLogin} className="form-grid" style={{ gridTemplateColumns: '1fr', gap: 'var(--space-4)' }}>
          ...
          <div className="field">
            ...
            <input 
              type="email" 
              placeholder={loginType === 'admin' ? "admin@cyvanta.com" : "student@cyvanta.com"}
              value={email}
